...
**Confirmação do professor sobre products.json vs. banco de dados (26/08):** Gabriel perguntou direto pro Johni (contato oficial do desafio) se, optando por banco de dados real, o `products.json` continuava obrigatório — resposta: não, pode desconsiderar, os produtos ficariam no banco. Isso resolve a dúvida que já estava registrada aqui ("se confirmar a exceção por escrito, reavaliar"). Decisão tomada com Gabriel na hora (26/08): **manter o `products.json` como está por enquanto**, sem migrar pra banco de dados naquele momento — o risco de não terminar a tempo pesou mais que o ganho.

**Atualização (27/08): decisão revisada — Gabriel quer migrar pra Supabase.** Com o resto do escopo pronto (carrinho, login, checkout, dark mode, código comentado, loja no ar), ele decidiu retomar a ideia. Ver item em "Extras" acima e a explicação técnica já dada sobre o que muda (`fetch("data/products.json")` vira consulta numa tabela; `obterCarrinho`/`salvarUsuario`/etc. de `shared.ts` poderiam virar chamadas assíncronas ao Supabase). Ainda não implementado — Gabriel vai avisar quando quiser começar.
...
**Dark mode implementado e testado (27/08):** botão no cabeçalho (ícone de lua/sol) alternável em todas as 6 páginas do site, salvo em `localStorage` (`pijamoon_tema`) e aplicado antes do CSS renderizar (script inline no `<head>` de cada página) pra não "piscar" claro e trocar pra escuro logo depois.
...
**Código inteiro comentado pra estudo (27/08):** Gabriel pediu pra estudar o código arquivo por arquivo antes da call/vídeo. Todos os `.ts`, os 6 `.html` e o topo do `style.css` ganharam comentários explicando o que cada parte faz e por quê — sem mudar nenhum comportamento (testado de ponta a ponta depois: busca, filtro, hero, carrinho, login, checkout com cartão e Pix). Cada `ts/*.ts` agora começa com um comentário de cabeçalho explicando o papel do arquivo; `style.css` ganhou um sumário no topo listando a ordem das 12 seções. Ordem de leitura combinada: `shared.ts` → `app.ts` → (`produto/index.html` + `produto.ts`) → (`carrinho/index.html` + `carrinho.ts`) → (`login/index.html` + `login.ts`) → (`checkout/index.html` + `checkout.ts`) → `como-fiz/index.html` → `style.css` por último.
